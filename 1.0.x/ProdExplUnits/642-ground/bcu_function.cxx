...
int ResetPcuRequestWhenOtheGroundBcuIsD2p(int pcu_no,int car_no)
{
	common_big_package_t BigConmInfo_temp;
	BigConmInfo_temp.common_big_data_u.seat_id=bcu_state.bcu_info.devices_no;
	BigConmInfo_temp.common_big_data_u.iph_refuse_no = pcu_no;
	BigConmInfo_temp.common_big_data_u.car_no=car_no;
	return IphDeleteLink(&BigConmInfo_temp);
}
...
static int BcuUpdateLink(const common_big_package_t *p_BigConmInfo_temp)
{
	...
	Node *temp;
	int vn=p_BigConmInfo_temp->common_big_data_u.car_no;
	int bcu_receive_no=p_BigConmInfo_temp->common_big_data_u.bcu_receive_no;
	diag_printf("BcuUpdateLink-vn=%d-bcu_receive_no=%d\n",vn,bcu_receive_no);
	diag_printf("BcuUpdateLink--%d-%d\n",p_BigConmInfo_temp->common_big_data_u.seat_id,bcu_state.bcu_info.devices_no);
	temp=update_list(BCURequsthead,vn,bcu_receive_no,1);
	if( temp == NULL )
	{
	...
	if(p_BigConmInfo_temp->common_big_data_u.seat_id!=bcu_state.bcu_info.devices_no)
	{
		BCURequsthead = deletes_list( BCURequsthead, p_BigConmInfo_temp->common_big_data_u.bcu_receive_no, p_BigConmInfo_temp->common_big_data_u.car_no);
		ret= dispalys(BCURequsthead);//显示请求，返回请求数
		if(ret == 0)
		{
			bcu_state.bcu_request_number=ret;
...
int ProbeBigCommPackage(const common_big_package_t *p_BigConmInfo)
{
	...
	diag_printf("Probe big package bcu_receive_no=%d\n",p_BigConmInfo->common_big_data_u.bcu_receive_no);
	...
	switch(common_type_package)
	{
		case 2:
			if(p_BigConmInfo->common_big_data_u.seat_id == bcu_state.bcu_info.devices_no)
			{
				...
				{
					diag_printf("iscs enter control ann to car,bcu_state.mic_owner = %d\n",bcu_state.mic_owner);
		...
		break;
		case 5:
			diag_printf("iscs enter control iph monitor to car.seat_id=%d,%d\n",p_BigConmInfo->common_big_data_u.seat_id,bcu_state.bcu_info.devices_no);
			if(p_BigConmInfo->common_big_data_u.seat_id == bcu_state.bcu_info.devices_no)
			{
		...
		break;
		case 6:
			diag_printf("iscs enter control iph recep to car.seat_id=%d,%d\n",p_BigConmInfo->common_big_data_u.seat_id,bcu_state.bcu_info.devices_no);
			if(p_BigConmInfo->common_big_data_u.seat_id == bcu_state.bcu_info.devices_no)
			{
				if(p_BigConmInfo->common_big_data_u.intercom_event_flag==1)
				{
					diag_printf("iscs receive PCU--%d\n",p_BigConmInfo->common_big_data_u.iph_receive_no);
					SetReceptPCUNo_ISCS(p_BigConmInfo->common_big_data_u.iph_receive_no,bcu_state.bcu_info.devices_no);
				    RecvD2PRequest();
					BlockBufferWrite(bcu_state.comm_server_recv_big_buffer_id,iph_select_intercom,sizeof(common_big_package_t));
				}
				if(p_BigConmInfo->common_big_data_u.intercom_event_flag==0)
				{
					diag_printf("iscs refuese PCU--%d\n",p_BigConmInfo->common_big_data_u.iph_refuse_no);
					if(bcu_state.bcu_active_intercom_state->state_id == D2P_INTERCOMM_EVENT && is_intercomming_with_pcu == 1)
					{
						CloseAudioSampleTimer();
					}
					iph_select_intercom->pkg_type=10;
					iph_select_intercom->common_big_data_u.car_no=p_BigConmInfo->common_big_data_u.car_no;
//					BlockBufferWrite(bcu_state.comm_server_send_big_buffer_id,iph_select_intercom,sizeof(common_big_package_t));
					cyg_thread_delay(10);
					RefuseD2PRequest();
				}
			}
			if(p_BigConmInfo->common_big_data_u.seat_id != bcu_state.bcu_info.devices_no  && p_BigConmInfo->common_big_data_u.intercom_event_flag == 1)
			{
				diag_printf("iscs receive PCU--%d,car_no:%d,%dwehen ResetPcuRequestWhenOtheGroundBcuIsD2p\n",
						p_BigConmInfo->common_big_data_u.iph_receive_no,p_BigConmInfo->common_big_data_u.car_no,sizeof(common_big_package_t));
				bcu_state.pcu_request_info.request_number=ResetPcuRequestWhenOtheGroundBcuIsD2p(p_BigConmInfo->common_big_data_u.iph_receive_no,p_BigConmInfo->common_big_data_u.car_no);
			}
		break;
		case 7:
