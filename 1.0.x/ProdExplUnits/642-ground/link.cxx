...
Node * query_list(Node* head, int p_vehicle_number,int p_device_id, Node **precursor )
{
	...
	*precursor = head;
	...
	while(temp != NULL)
	{
		printf("%s-%d-%d-%d-%d\n",__FUNCTION__,temp->devices_id , p_device_id,temp->vehicle_number, p_vehicle_number);
		if((temp->devices_id == p_device_id) && (temp->vehicle_number == p_vehicle_number))
		{

			  diag_printf("%s Successful lookup ! \n",__FUNCTION__);
