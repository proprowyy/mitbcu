...
/*Add sound header*/
int MicAdPcmWavheader2(int buffer_id)
{
        int ret = 0;
...
int OpenSndCard(cyg_io_handle_t *param_audio_handle,char *snd_mode,int *current_sounder_mode)
{
	int ret = 0;
...
void bcu_6d5w_ctrl_io(unsigned char dia)
{
     if( dia==0 )
     {
	hal_gpio_pin_clear( CYGHWR_HAL_KINETIS_PIN(E, 16, 1, KINETIS_PIN_PULLUP) );          
	hal_gpio_pin_clear( CYGHWR_HAL_KINETIS_PIN(E, 17, 1, KINETIS_PIN_PULLUP) ); 	
     }
     else if( dia==1 )
     {
	hal_gpio_pin_clear( CYGHWR_HAL_KINETIS_PIN(E, 16, 1, KINETIS_PIN_PULLUP) );          
