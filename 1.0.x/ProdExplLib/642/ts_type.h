...
typedef struct
{
	unsigned char total_bcu_number;//PCU总算
	unsigned char recept_bcu_no;//拒绝PCU的号码
	unsigned char refuse_bcu_no;//接通的PUC号码
	unsigned char request_number;//PCU请求的顺序
	unsigned char hang_up_bcu_no;//挂起的PCU号码
	unsigned char bcu_request_table[30];//PCU请求表
	unsigned char open_snd_playback; ///< add, 0107	
	unsigned char bcu_alarm_playing; ///< add, 0107	
	unsigned char bcu_alarm_playing_again; ///< add, 0107
	unsigned char bcu_connection_status[30];//PCU接通状态表
	unsigned char bcu_connection_number;
	unsigned char current_conect_bcu_no;//当前接通PCU号码
	unsigned char bcu_request_name[20];
 	unsigned char bcu_request_dev_no;
}bcu_request_info_t;//PCU请求记录表
